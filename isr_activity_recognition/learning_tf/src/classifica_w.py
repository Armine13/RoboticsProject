...
def callback(data):
    ...
    rospy.loginfo(rospy.get_caller_id() + " button pressed: %s", data.buttons)
...
#    rospy.Subscriber('recognizer/output', String, callback)
    rospy.Subscriber("joy", Joy, callback,  queue_size=100)
...
treino = sio.loadmat('/home/mscv/ros/indigo/catkin_ws/src/isr_activity_recognition/learning_tf/src/treino_torso_camera.mat')
treino = treino['treino']
#maxval = np.max(treino.max(0))
#minval = np.min(treino.min(0))
maxval = treino.max(0)
minval = treino.min(0)
...
while not rospy.is_shutdown():
    ...
    print('min', minval.shape)
    print('min', maxval.shape)
    ...
    # NAIVE BAYES
    clfNB = externals.joblib.load('/home/mscv/ros/indigo/catkin_ws/src/isr_activity_recognition/learning_tf/src/NaiveBayes_torso_camera/NB_clf.pkl')
    ...
    # SVC
    clf_svc = externals.joblib.load('/home/mscv/ros/indigo/catkin_ws/src/isr_activity_recognition/learning_tf/src/SVC_torso_camera/svm_clf.pkl')
    ...
    if segundos>0:
        ...
        conta_class = np.zeros((8)) 
        if proba_final.index(max(proba_final)) == 0: # Se running
            os.system("rosrun sound_play say.py 'Walking'")
        if proba_final.index(max(proba_final)) == 1: # Se running
            os.system("rosrun sound_play say.py 'Standing still'")
        if proba_final.index(max(proba_final)) == 2: # Se running
            os.system("rosrun sound_play say.py 'Working on computer'")
        if proba_final.index(max(proba_final)) == 3: # Se running
            os.system("rosrun sound_play say.py 'Talking on the phone'")
        if proba_final.index(max(proba_final)) == 4: # Se running
            os.system("rosrun sound_play say.py 'Running'")
        if proba_final.index(max(proba_final)) == 5: # Se running
            os.system("rosrun sound_play say.py 'Jumping'")
        if proba_final.index(max(proba_final)) == 6: # Se running
            os.system("rosrun sound_play say.py 'Falling'")
        if proba_final.index(max(proba_final)) == 7: # Se running
            os.system("rosrun sound_play say.py 'Sitting down'")
